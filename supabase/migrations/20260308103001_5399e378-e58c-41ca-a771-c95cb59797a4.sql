...
-- Update generar_plan_1603 to use overlapping accompaniment windows
...
BEGIN
  IF NEW.bajo_pe_1603 = true AND NEW.fecha_primer_servicio IS NOT NULL THEN
    IF NOT EXISTS (SELECT 1 FROM expedientes_1603 WHERE maquinista_id = NEW.id AND estado = 'abierto') THEN
      ...
      -- ACOMPAÑAMIENTOS: overlapping cumulative windows
      INSERT INTO plan_1603 (expediente_id, tipo, etiqueta, orden, mes, inicio_ventana, fin_ventana) VALUES
        ...
        (v_expediente_id, 'acompanamiento', 'Primer Semestre', 3, 6, v_fecha_origen, v_fecha_origen + INTERVAL '182 days'),
...
-- Update recalcular_plan_1603 to use overlapping accompaniment windows
...
BEGIN
  ...
  UPDATE public.plan_1603 SET etiqueta = 'Primer Semestre', orden = 3, mes = 6, inicio_ventana = v_fecha_origen, fin_ventana = v_fecha_origen + INTERVAL '182 days' WHERE expediente_id = _expediente_id AND tipo = 'acompanamiento' AND orden = 3;
...
-- Fix existing expedientes: update accompaniment windows to be cumulative
UPDATE public.plan_1603
SET inicio_ventana = (
  SELECT e.fecha_primer_servicio
  FROM public.expedientes_1603 e
  WHERE e.id = plan_1603.expediente_id
)
WHERE tipo = 'acompanamiento' AND orden IN (2, 3);
